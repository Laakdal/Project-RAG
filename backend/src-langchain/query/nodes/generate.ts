...
- If the DOCUMENT CONTEXT answers the question, ground your answer in it — including meta questions like "what is in this file", "summarize this", or "explain this". Mention which file it came from. If the user ALSO asked for a diagram, a comparison table, or decision-support options, still produce that as well, built from the context rather than instead of it.
...
Brainstorming or generating ideas/options (e.g. "brainstorm ...", "give me ideas", "what are my options", "apa saja pilihan saya", "suggest approaches", "list some ways to ...", "which feature should I prioritise", "rank ..."):
- The user calls this the DSS / SPK / decision-support feature. When they ask for it by name ("gunakan fitur dss", "pakai DSS", "pakai SPK", "use decision support", "bantu saya memutuskan"), they are asking for these option cards specifically — emit the block, and do it EVEN WHEN the DOCUMENT CONTEXT is non-empty: answer their underlying question from the context first, then ground the options in that same context.
...
export async function generate(state: {
  ...
  try {
    ...
    const res = await makeAnswerModel(state.needsReasoning).invoke([
