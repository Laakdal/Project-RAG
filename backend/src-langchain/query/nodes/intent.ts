...
Attached files take precedence: when "Attached file(s) in this chat" below says yes, the user has uploaded a file to THIS chat and it is their primary source. For questions about document content, set useDrive=false and let the attachment answer them — do NOT send the question out to their Google Drive. With a file attached, set useDrive=true ONLY when the user clearly points somewhere else: their Drive or library by name, their organization's records, or a specific OTHER document, reference number, or internal code (PPAB / SPPD / DTIS). 'bagaimana cara menambahkan non fungsionalnya' with a file attached -> useDrive=false; 'apa isi SOP IT Project Management' with a file attached -> useDrive=true.
...
Examples (question -> useDrive,webSearch,needsReasoning,needsOptions): 'apa itu css' -> false,false,false,false; 'jelaskan konsep agile' -> false,false,false,false; 'bandingkan agile dan waterfall' -> false,false,true,true; 'mana yang lebih baik, paket A atau paket B' -> true,false,true,true; 'harga saham terbaru Apple' -> false,true,false,false; 'berita terbaru soal AI' -> false,true,false,false; 'siapa CEO OpenAI saat ini' -> false,true,false,false; 'apa itu palmco' -> true,false,false,false; 'PPAB tanggal 7 juli 2025' -> true,false,false,false; 'surat dinas nomor 110/VII/2025' -> true,false,false,false; 'SPPD Jakarta' -> true,false,false,false; 'foto apa ini' -> false,false,false,false; 'jelaskan gambar ini' -> false,false,false,false; 'buatkan flowchart login' -> false,false,false,false; 'apa isi SOP IT Project Management' -> true,false,false,false; 'ringkas laporan keuangan Q3' -> true,false,false,false; 'halo' -> false,false,false,false; 'tolong bantu dengan skripsi saya' -> false,false,false,true.
With a file attached to this chat: 'bagaimana cara menambahkan non fungsionalnya' -> false,false,false,true; 'apa isi dokumen ini' -> false,false,false,false; 'ringkas file ini' -> false,false,false,false.`;
...
export async function intent(state: {
  ...
  // Awaited BEFORE the classifier call rather than alongside it: the routing
  // rule "an attached file outranks Drive" is useless unless the classifier is
  // told whether there is one. It is a single indexed count, so the serialisation
  // costs a few milliseconds against an LLM round trip.
  const hasAttached = await attachedPromise;
  try {
    ...
    const content =
      `${INTENT_INSTRUCTIONS}\n\n` +
      `Attached file(s) in this chat: ${hasAttached ? "yes" : "no"}\n\n` +
      `Attached file(s) text (may be empty):\n${attached}\n\n` +
